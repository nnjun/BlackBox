...
//        ResolveInfo resolveInfo = resolveService(intent, resolvedType, userId);
    ...
    public Intent bindService(Intent intent, final IBinder binder, String resolvedType, int userId) {
        ...
        final Intent stubServiceIntent = createStubServiceIntent(intent, serviceInfo, processRecord, runningServiceRecord.mStartId.get());
        return stubServiceIntent;
    }
    ...
    public void onServiceDestroy(Intent proxyIntent, int userId) {
        if (proxyIntent == null)
            return;
        StubServiceRecord stubServiceRecord = StubServiceRecord.create(proxyIntent);
        mRunningServiceRecords.remove(new Intent.FilterComparison(stubServiceRecord.mServiceIntent));
    }
    ...
    public UnbindRecord onServiceUnbind(Intent proxyIntent, int userId) throws RemoteException {
        if (proxyIntent == null)
            return null;
        StubServiceRecord stubServiceRecord = StubServiceRecord.create(proxyIntent);
        ComponentName component = stubServiceRecord.mServiceIntent.getComponent();

        RunningServiceRecord runningServiceRecord = findRunningServiceRecord(stubServiceRecord.mServiceIntent);
        if (runningServiceRecord == null)
            return null;
    ...
    private Intent createStubServiceIntent(Intent targetIntent, ServiceInfo serviceInfo, ProcessRecord processRecord, int startId) {
        ...
        ComponentName stubComp = new ComponentName(BlackBoxCore.getHostPkg(), StubManifest.getStubService(processRecord.vpid));
        stub.setComponent(stubComp);
        stub.setAction(UUID.randomUUID().toString());
        StubServiceRecord.saveStub(stub, targetIntent, serviceInfo, processRecord.userId, startId);
        return stub;
    }
    ...
    public IBinder peekService(Intent intent, String resolvedType, int userId) {
        ...
        if (processRecord == null)
            return null;
        try {
            return processRecord.client.peekService(intent);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return null;
