...
public class ActivityStack {
    ...
    private Intent startActivityProcess(int userId, Intent intent, ActivityInfo
        ...
        return getStartStubActivityIntentInner(intent, targetApp.vpid, userId, stubRecord, info);
    }
    ...
    private int realStartActivityLocked(IInterface appThread, Intent intent, String
            resolvedType,
                                        IBinder resultTo, String resultWho, int requestCode, int flags,
                                        Bundle options) {
        Class<?>[] classes = IActivityManager.startActivity.paramList();
    ...
    private Intent getStartStubActivityIntentInner(Intent intent, int vpid,
                                                   int userId, StubActivityRecord target,
                                                   ActivityInfo activityInfo) {
        Intent shadow = new Intent();
        shadow.setComponent(new ComponentName(BlackBoxCore.getHostPkg(), StubManifest.getStubActivity(vpid)));
        StubActivityRecord.saveStub(shadow, intent, target.mActivityInfo, target.mActivityRecord, target.mUserId);
        return shadow;
    }
            ...
            for (ActivityRecord activity : task.activities) {
                if (activity.userId == userId) {
                    if (activity.finished) {
                        try {
                            activity.processRecord.client.finishActivity(activity.token);
                        } catch (RemoteException e) {
                            e.printStackTrace();
                        }
                    }
